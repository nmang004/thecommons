...
/**
 * POST /api/manuscripts/:manuscriptId/invitations
 * Creates and sends a new invitation to a reviewer for a specific manuscript
 */
export async function POST(
  ...
  try {
    ...
    if (!requestData.reviewerIds || requestData.reviewerIds.length === 0) {
      return NextResponse.json(
        { error: 'reviewerIds are required' },
        { status: 400 }
      )
    }
...
/**
 * GET /api/manuscripts/:manuscriptId/invitations  
 * Get invitation statistics and details for a specific manuscript
 */
export async function GET(
  ...
  try {
    ...
    // Verify user has access to manuscript
    ...
    const { data: manuscript } = await supabase
      .from('manuscripts')
      .select('id, assigned_editor, author_id')
      .eq('id', manuscriptId)
      .single()
    ...
    const isAuthor = manuscript.author_id === user.id
    const isAssignedEditor = manuscript.assigned_editor === user.id

    if (!isEditor && !isAuthor && !isAssignedEditor) {
      return NextResponse.json(
        { error: 'Forbidden' },
        { status: 403 }
      )
