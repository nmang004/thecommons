  ...
  try {
    ...
    try {
      ...
      const editorNotificationTitle = payload.decision === 'accept'
        ? `Review Accepted: ${invitation.manuscripts.title}`
        : `Review Declined: ${invitation.manuscripts.title}`

      const editorNotificationMessage = payload.decision === 'accept'
        ? `${invitation.profiles.full_name} has accepted the review invitation for "${invitation.manuscripts.title}".`
        : `${invitation.profiles.full_name} has declined the review invitation for "${invitation.manuscripts.title}". Reason: ${payload.declineReason}`

      await notificationService.sendNotification({
        channels: {
          email: {
            to: invitation.invited_by_profile.email,
            ...
            body: `
              ...
              <ul>
                <li><strong>Name:</strong> ${invitation.profiles.full_name}</li>
                <li><strong>Email:</strong> ${invitation.profiles.email}</li>
    ...
    return NextResponse.json({
      ...
      ...(payload.decision === 'accept' && {
        nextSteps: {
          ...
          contactEditor: invitation.invited_by_profile.email
        }
      })
    })
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      reviewerName: (invitation.profiles as any).full_name,
